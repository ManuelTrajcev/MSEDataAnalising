...
const LandingPageTable = () => {
    ...
    const [selectedView, setSelectedView] = useState("risers");
    const selectedCompanyCode = "ALK,GRNT,KMB,MPT,REPL,SBT,STB,TEL,TTK";

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_BASE_URL}:8000/datascraper/api/get-last-day-data/?company_codes=${selectedCompanyCode}`);
                const responseData = await response.json();
                setData(responseData.map((entry) => ({
    ...
    const risers = data.filter((row) => parseFloat(row.percentage.replace(',', '.')) > 0);
    const fallers = data.filter((row) => parseFloat(row.percentage.replace(',', '.')) < 0);
                                ...
                                <tr key={index}>
                                    <td>
                                        <span className={`indicator ${
                                            ...
                                            {parseFloat(row.percentage.replace(",", ".")) > 0 && <span className="arrow up">↑</span>}
                                            {parseFloat(row.percentage.replace(",", ".")) < 0 && <span className="arrow down">↓</span>}
                                            {parseFloat(row.percentage.replace(",", ".")) === 0 && <span className="arrow neutral">•</span>}
                                            {row.company_code}
                                        </span>
                                    </td>
